...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            self.stdout.write(
                f"Start calculate next value for run_id based on execution_log table "
            )
            start = datetime.now()

            cursor.execute(
                "select coalesce(max(run_id::int) +1, 1) from log.execution_log"
            )
            run_id = cursor.fetchone()[0]

            finished_message = (
                f"Finished run_id calculation in"
                f" {(datetime.now() - start).seconds} seconds"
            )
            self.stdout.write(finished_message)

        # target_table := 'continuousaggregate_cmsa15min',
        complete_query = f"""call prc.proc_pre_post_process (
        ...
        with connection.cursor() as cursor:
            self.stdout.write(
                f"Start completing aggregation table {Cmsa15Min._meta.db_table}"
            )
            start = datetime.now()
            cursor.execute(complete_query)
            finished_message = (
                f"Finished completing aggregation table {Cmsa15Min._meta.db_table} in"
                f" {(datetime.now() - start).seconds} seconds"
            )
            self.stdout.write(finished_message)
