...
with RawData as(
    select
        sensor,
        timestamp,
        coalesce(avg((detail_elems ->> 'count')::integer) filter (where detail_elems ->> 'direction' = 'down'), 0) + coalesce(avg((detail_elems ->> 'count')::integer) filter (where detail_elems ->> 'direction' = 'up'), 0) as total_count,
        coalesce(avg((detail_elems ->> 'count')::integer) filter (where detail_elems ->> 'direction' = 'down'), 0) as count_down,
        coalesce(avg((detail_elems ->> 'count')::integer) filter (where detail_elems ->> 'direction' = 'up'), 0) as count_up,
