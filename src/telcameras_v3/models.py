...
class Observation(models.Model):
    ...
    message_id = (
        models.IntegerField()
    )  # Coming from "id" in the message. It's an auto-increment per sensor / volgnummer.
    ...
    sensor_state = models.CharField(
        max_length=255
    )  # Coming from "status" e.g. "operational"
...
class GroupAggregate(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    observation_timestamp = TimescaleDateTimeField(
        interval="1 day", default=timezone.now
    )  # Copied from the observation
    azimuth = models.SmallIntegerField()
    count = models.SmallIntegerField()
    count_scrambled = models.SmallIntegerField(
        null=True
    )  # This is meant to use a slightly scrambled version of the count with a +1 or -1 for privacy reasons
...
class Person(models.Model):
    ...
    observation_timestamp = TimescaleDateTimeField(
        interval="1 day", default=timezone.now
    )  # Copied from the observation object
    ...
    type = models.CharField(
        max_length=255
    )  # e.g. "pedestrian" or "cyclist" or maybe something else
